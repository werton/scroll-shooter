// Sroll shooter example
//
// writen by werton playskin 05/2025
...
void InitPlayer(Player* player, u8 index) {
    ...
    GameObject_Init((GameObject *)player, &player_sprite, PAL1, FIX16(16), FIX16(SCREEN_HEIGHT/2 + index*48), 
...
void GameObject_Init(GameObject *object, const SpriteDefinition *spriteDef, u16 pal, fix16 x, fix16 y, u16 w, u16 h, s16 hp, s16 damage) {
    if (!object->sprite) {
        object->sprite = SPR_addSprite(spriteDef, F16_toInt(x), F16_toInt(y), TILE_ATTR(pal, FALSE, FALSE, FALSE));
    }
    else {
...
void Player_Explode(GameObject *object) {
    SpawnExplosion(object->x - FIX16(8), object->y);
    SPR_setFrame(object->sprite, 0);
    SPR_setVisibility(object->sprite, HIDDEN);
}
...
void UpdatePlayerEnemyCollision(Player* player) {
    FOREACH_ALLOCATED_IN_POOL(Enemy, enemy, enemyPool) {
        ...
        if (GameObject_IsCollided((GameObject *)player, (GameObject *)enemy)) {
            ...
            Player_Explode((GameObject *)player);
