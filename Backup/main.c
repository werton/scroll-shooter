...
typedef struct Player {
    ...
    bool isDamageable;  // Flag indicating if player can take damage (FALSE during invincibility)
...
void PlayerJoin_Update() {
    ...
    // If player doesn't exist and START is pressed on joypad
...
void GameObject_ReleaseWithExplode(GameObject *object, Pool *pool) {
    ...
    Explosion_Spawn(object->x - FIX16(8), object->y);
...
void Player_Explode(Player *player) {
    ...
    Explosion_Spawn(player->x - FIX16(8), player->y);
...
void EnemySpawner_Update() {
    ...
    if (game.wave.spawnedCount == game.wave.spawner->enemyCount) {
        EnemyWave_Switch();  // Switch to next pattern
    }   
}
...
void Player_UpdateEnemyCollision(Player* player) {
    // Skip collision check if player can't take damage
    if (!player->isDamageable)
        return;
...
void RenderMessage() {	   
        ...
        if (blinkCounter < 30) { 
			...
			FOREACH_PLAYER(player) {           	
	        	if (!player->hp)
					if (player->index == 0)
	            		VDP_drawTextBG(WINDOW, "1P PRESS START", 8, 27);
					else 
						VDP_drawTextBG(WINDOW, "2P PRESS START", 25, 27);	
   		 }
        }
        else
        	VDP_clearTextAreaBG(WINDOW, 8, 27, 32, 1);
